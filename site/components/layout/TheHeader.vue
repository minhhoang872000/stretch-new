        ...
        <div class="hidden lg:flex items-center gap-4">
          ...
          <!-- <div v-if="loggedIn" class="flex items-center gap-2">
            <img
              ...
              :src="user.avatar"
              :alt="user?.name"
              class="w-8 h-8 rounded-full object-cover border border-border-default"
            />
            <span class="text-xs font-heading font-semibold text-navy max-w-[100px] truncate">{{ user?.name }}</span>
            <button
              @click="logout"
              class="text-xs font-heading font-semibold text-text-secondary hover:text-accent transition-colors"
            >
              {{ t('nav.logout') }}
            </button>
          </div>
