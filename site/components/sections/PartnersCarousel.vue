...
const dotCount = computed(() => Math.ceil(totalSlides.value / visibleCount.value))
const activeDot = computed(() => Math.floor(currentIndex.value / visibleCount.value) % dotCount.value)

function scrollTo(index: number) {
  currentIndex.value = index % totalSlides.value
  if (trackRef.value) {
    const itemWidth = trackRef.value.scrollWidth / allPartners.length
    trackRef.value.style.transition = 'transform 0.8s cubic-bezier(0.16, 1, 0.3, 1)'
...
function prev() {
  scrollTo(currentIndex.value <= 0 ? totalSlides.value - 1 : currentIndex.value - 1)
}
  ...
  <section class="py-14 lg:py-20 bg-off-white">
    <div class="section-container">
      ...
      <div class="text-center mb-10 lg:mb-14">
        <span class="text-[11px] font-heading font-bold text-text-secondary uppercase tracking-[0.2em] block mb-3">
          {{ $t('partners.eyebrow') }}
        </span>
        <h2 class="text-2xl md:text-3xl lg:text-[34px] font-heading font-bold text-navy leading-tight mb-4">
          {{ $t('partners.title') }}
        </h2>
        <p class="text-sm md:text-base text-text-secondary max-w-xl mx-auto leading-relaxed">
          {{ $t('partners.subtitle') }}
        </p>
      </div>
      ...
      >
        ...
        <div class="overflow-hidden mx-14">
          ...
          >
            ...
            >
              <div class="flex items-center justify-center px-6 select-none cursor-default group/logo">
                <img
                  :src="partner.logo"
                  :alt="partner.name"
                  class="h-11 md:h-14 lg:h-18 w-auto object-contain grayscale opacity-60 group-hover/logo:grayscale-0 group-hover/logo:opacity-100 transition-all duration-300"
                />
              </div>
            </div>
          </div>
      ...
      <div class="flex items-center justify-center gap-2 mt-6">
        <button
          ...
          :class="activeDot === dotIndex ? 'bg-navy w-6' : 'bg-navy/15 hover:bg-navy/30'"
